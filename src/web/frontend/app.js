...
// Live clip identifiers are not unique across a Set, so the browser selects
// clips by their session slot instead.
function clipKey(trackId, sceneIndex) {
...
async function loadFile(file) {
  ...
  try {
    ...
    const firstClip = firstSourceClip();
    state.selectedClipKey = firstClip ? clipKey(firstClip.trackId, firstClip.sceneIndex) : null;
    renderProject();
  ...
  const rows = state.preview.sourceTracks.map((track) => {
    ...
    for (const clip of track.clips) {
      ...
      button.classList.toggle(
        "selected",
        clipKey(clip.trackId, clip.sceneIndex) === state.selectedClipKey,
      );
      ...
      button.addEventListener("click", () => {
        state.selectedClipKey = clipKey(clip.trackId, clip.sceneIndex);
        renderClipList();
        renderSelectedClip();
      });
...
function renderSelectedClip() {
  ...
  const sourceClip = findSourceClip(state.selectedClipKey);
  const targetClip = findTargetClip(state.selectedClipKey);
...
function findSourceClip(clipKeyValue) {
  return (
    state.preview?.sourceTracks
      .flatMap((track) => track.clips)
      .find((clip) => clipKey(clip.trackId, clip.sceneIndex) === clipKeyValue) ?? null
  );
}

function findTargetClip(clipKeyValue) {
  return (
