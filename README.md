...
Install `wasm-pack`, build the browser package into `target/`, stage the static
assets next to it, and serve the staged folder from a local HTTP server:
...
npx serve target/browser-preview
```

Open the printed local URL and drop an `.als` file onto the page. The browser reads
the selected bytes locally and does not upload the Set. Everything under `target/`
is a local build artifact and should not be committed.

For a no-server build that opens directly from disk, bundle the same output into
...
This writes `target/able-converter-preview.html`, which is likewise a local build
